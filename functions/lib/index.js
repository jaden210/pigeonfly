...
exports.inviteNewUser = functions.firestore
  ...
  .onCreate(snapshot => {
    ...
    const mailTransport = nodemailer.createTransport(
      `smtps://jaden210@gmail.com:FordGT-40@smtp.gmail.com`
    );
    const mailOptions = {
    ...
    mailOptions.subject =
      "You’re a new member of  Compliancechimp with " +
      info.companyName +
      " - Please see how to join your team below.";
    mailOptions.html = `Hi ${info.inviteName}<br><br>
    ${
      info.companyName
    } is using Compliancechimp to manage time tracking, worksite logs, and safety training. Please visit the App Store or Goolge Play Stopre to download the free app and join your team now.
...
/*    --------- ACHIEVEMENTS ----------     */
exports.achievementTeam = functions.firestore
  ...
  .onUpdate((change, context) => {
    ...
    if (!oldTeam.logoUrl && newTeam.logoUrl) {
      updateCompletedAchievement(newTeam.id, "hasCompanyLogo", true)
        .then(() => console.log("successs"))
        .catch(error => console.log(error));
    }
  });
exports.achievementProfile = functions.firestore
  ...
  .onUpdate((change, context) => {
    ...
    if (
      oldUser.accountType == "owner" &&
      !oldUser.profileUrl &&
      newUser.profileUrl
    ) {
      updateCompletedAchievement(newUser.teamId, "hasOwnerProfileUrl", true)
        .then(() => console.log("successs"))
        .catch(error => console.log(error));
    }
  });
exports.achievementLogs = functions.firestore
  .document("log/{id}")
  .onCreate(snapshot => {
    let log = snapshot.data();
    /* total logs achievement */
    updateCompletedAchievement(log.teamId, "logsCount", 1, true)
      .then(() => console.log("successs"))
      .catch(error => console.log(error));
  });
exports.achievementTimeclocks = functions.firestore
  .document("timeclock/{id}")
  .onCreate(snapshot => {
    let timeclock = snapshot.data();
    /* total timeclock achievement */
    updateCompletedAchievement(timeclock.teamId, "timeclocksCount", 1, true)
      .then(() => console.log("successs"))
      .catch(error => console.log(error));
  });
exports.achievementinvitedUsers = functions.firestore
  .document("invitation/{id}")
  .onCreate(snapshot => {
    let invitation = snapshot.data();
    /* total invites achievement */
    updateCompletedAchievement(invitation.teamId, "invitedUsers", 1, true)
      .then(() => console.log("successs"))
      .catch(error => console.log(error));
  });
exports.achievementSelfAssesment = functions.firestore
  .document("self-inspection/{id}")
  .onCreate(snapshot => {
    let selfInspection = snapshot.data();
    /* total self inspections achievement */
    updateCompletedAchievement(
      selfInspection.teamId,
...
exports.achievementFinishSelfInspection = functions.firestore
  .document("self-inspection/{id}")
  .onUpdate((change, context) => {
    ...
    newSI.selfInspections.forEach(nInspection => {
      oldSI.selfInspections.forEach(oInspection => {
        if (
          nInspection.completedAt !== null &&
          oInspection.completedAt == null
        ) {
          // has been completed
          updateCompletedAchievement(
            newSI.teamId,
...
exports.achievementInjuryReport = functions.firestore
  .document("injury-report/{Id}")
  .onCreate(snapshot => {
    let injuryReport = snapshot.data();
    /* total self inspections achievement */
    updateCompletedAchievement(injuryReport.teamId, "injuryReports", 1, true)
      .then(() => console.log("successs"))
      .catch(error => console.log(error));
  });
    ...
    .then(querySnapshot => {
      querySnapshot.forEach(doc => {
        ...
        return admin
          ...
          .update(obj)
          .then(() =>
            console.log(mapKey + " has been updated in completed-achievement")
          )
          .catch(error => console.log(error));
      });
    })
