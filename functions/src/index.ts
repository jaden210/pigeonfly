...
exports.inviteNewUser = functions.firestore
  ...
  ${
    info.companyName
  } is using Compliancechimp to manage safety training, worksite logs, record keeping, and more, as part of an ongoing commitment to safety and compliance. You've been invited to the team using ${
      info.inviteEmail
...
exports.createdLog = functions.firestore
  ...
  .onCreate((snapshot, context) => {
    ...
    return Promise.all([event, achievement]).then(() =>
      console.log("create logs complete")
    );
  });
...
exports.createdTimeclock = functions.firestore
  ...
  .onCreate((snapshot, context) => {
    ...
    return Promise.all([event, achievement]).then(() =>
      console.log("create timeclock complete")
    );
  });
...
exports.modifiedTimeclock = functions.firestore
  ...
  .onUpdate((change, context) => {
    ...
    if (!oldTime.updatedId && newTime.updatedId) {
      let updatedEvent = logAsEvent(
        EventType.timeclock,
        EventAction.updated,
        change.after.id,
        newTime.updatedId,
        "modified a timeclock",
        context.params.teamId
      );
  ...
  .onCreate(snapshot => {
    ...
    promises.push(
      logAsEvent(
        ...
        `${invitation.inviteName} was invited to the Team`,
    ...
    return Promise.all(promises).then(() =>
      console.log("created invitation complete")
    );
  });
...
exports.likedTraining = functions.firestore
  ...
  .onCreate((snapshot, context) => {
    const achievement = updateCompletedAchievement(
      context.params.teamId,
      "likedTrainingContent",
      1,
      true
    );

    return Promise.all([achievement]).then(() =>
      console.log("created injury report complete")
    );
  });
...
exports.createdSurvey = functions.firestore
  ...
  .onCreate((snapshot, context) => {
    ...
    return Promise.all([log, achievement]).then(() =>
...
exports.createdSurveyResponse = functions.firestore
  ...
  .onCreate((snapshot, context) => {
    ...
    return Promise.all([trainingLog, trainingSurveyResponse]).then(() =>
