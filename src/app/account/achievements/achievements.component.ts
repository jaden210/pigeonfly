...
export class AchievementsComponent implements OnInit, OnDestroy {
              ...
              level.checkpoints.forEach(checkpoint => {
                ...
                checkpoint.achievements.forEach(achievement => {
                  ...
                  if (
                    ...
                    if (
                      moment().diff(date, "days") <= achievement.completedValue
                    ) {
                ...
                checkpoint.progress =
                  checkpoint.progress / checkpoint.achievements.length + "%";
              });
              if (
                level.completedAchievementsCount ==
                level.possibleAchievementsCount
  ...
  routeOrPop(badge) {
    if (badge.routerLink) {
      ...
      dialog.afterClosed().subscribe(key => {
        if (key) {
          // save the date to completedAchievements
          let date = new Date();
          this.accountService.db
            .collection("completed-achievement")
            .doc(this.completedAchievementsDocId)
            .update({ [key]: date });
        }
      });
    }
  ...
  getAchievements() {
    let achievementsCollection = this.accountService.db.collection(
      "achievement",
  ...
  getCompletedAchievements() {
    let completedCollection = this.accountService.db.collection(
      "completed-achievement",
      ref => ref.where("teamId", "==", this.accountService.aTeam.id)
    );
