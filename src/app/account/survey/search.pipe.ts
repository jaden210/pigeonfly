...
import { Survey } from '../account.service';
    ...
    if (filter) {
      ...
      for (let survey of surveys) {
        ...
        for (let f of filter) { // for value of filter array, built from arguments passed in
          if (survey.category.toLowerCase().includes(f.toLowerCase())) { // convert to lower case, compare keyword to f
              rv.push(survey); // append matching store to results[]
              continue cardLoop; // if block true, run filters loop again
          }
          if (survey.title.toLowerCase().includes(f.toLowerCase())) { // convert to lower case, compare keyword to f
              rv.push(survey); // append matching store to results[]
              continue cardLoop; // if block true, run filters loop again
          }
        }
