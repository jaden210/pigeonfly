...
export class LoginComponent implements OnInit {
  ...
  checkEmail() {
    this.loginErrorStr = "";
    this.auth.auth.fetchSignInMethodsForEmail(this.login.email).then(
      data => {
        if (data.length > 0) {
          // they exist
          this.isVerified = true;
        } else {
          // no account, they are looking to join a team.
          let inviteCollection = this.appService.db.collection(
            "invitation",
            ref =>
              ref.where("inviteEmail", "==", this.login.email.toLowerCase())
          );
          inviteCollection.valueChanges().subscribe((invites: any) => {
            if (invites.length > 0) {
              ...
              // who are you??
              this.loginErrorStr =
                "We don't know who you are. if you want to create a team, click Create team below";
            }
          });
        }
      },
